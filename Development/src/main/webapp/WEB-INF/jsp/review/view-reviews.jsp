    ...
    <p class="page-subtitle">
        <c:choose>
            <c:when test="${not empty reviews}">
                ${fn:length(reviews)} verified review<c:if test="${fn:length(reviews) != 1}">s</c:if> from happy customers
            </c:when>
...
<c:choose>
    <c:when test="${empty reviews}">
        <div class="empty-state">
            ...
            <br/>
            <a href="${pageContext.request.contextPath}/reviews/submit" class="btn btn-primary">Be the First to Review</a>
        </div>
    </c:when>
    <c:otherwise>
