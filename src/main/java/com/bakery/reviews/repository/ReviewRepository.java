package com.bakery.reviews.repository;
...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT r FROM Review r WHERE r.reviewType = 'PRODUCT' AND r.productName = :productName AND r.status = com.bakery.reviews.model.Review.ReviewStatus.APPROVED")
    List<Review> findApprovedByProductName(@Param("productName") String productName);

    @Query("SELECT AVG(r.rating) FROM Review r WHERE r.reviewType = 'PRODUCT' AND r.productName = :productName AND r.status = com.bakery.reviews.model.Review.ReviewStatus.APPROVED")
