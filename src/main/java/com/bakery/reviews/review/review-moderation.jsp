    ...
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        <%@ taglib prefix="fn" uri="jakarta.tags.functions" %>
            ...
            <div class="stats-row">
                ...
                <div class="stat-card">
                    <div class="stat-number" style="color:#f0a030;">${pendingCount}</div>
                    <div class="stat-label">Awaiting Review</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number" style="color:#3da870;">${approvedCount}</div>
                                ...
                                <c:forEach var="r" items="${pendingReviews}">
                                    <tr>
                                        <td>
                                            ...
                                            <small style="color:var(--rv-text-secondary);">${r.customerEmail}</small>
                                        </td>
                                        <td>
                                            <c:choose>
                                        ...
                                        <td>
                                            ...
                                            <small style="color:var(--rv-text-secondary);"> ${r.rating}/5</small>
                                        </td>
                                        <td class="comment-cell" title="${r.comment}">"${r.comment}"</td>
                                        <td>
                                            <span data-date="${r.createdAt}"
                                                style="font-size:0.8rem;color:var(--rv-text-secondary);">${r.createdAt}</span>
                                        </td>
                                        <td>
                                            <div class="action-group">
                                                <form
                                                    action="${pageContext.request.contextPath}/admin/reviews/${r.id}/approve"
                                                    method="post">
                                                    <button type="submit" class="btn btn-success btn-sm">✔
                                                        Approve</button>
                                                </form>
                                                <form
                                                    action="${pageContext.request.contextPath}/admin/reviews/${r.id}/reject"
                                                    method="post">
                                                    <button type="submit" class="btn btn-warning btn-sm">✘
                                                        Reject</button>
                                                </form>
                                                <form
                                                    action="${pageContext.request.contextPath}/admin/reviews/${r.id}/delete"
                                                    method="post" data-confirm="Permanently delete this review?">
                            ...
                            <table class="review-table">
                                ...
                                <tbody>
                                    <c:forEach var="r" items="${allReviews}">
                                        <tr>
                                            <td style="color:var(--rv-text-secondary);font-size:0.8rem;">#${r.id}</td>
                                            ...
                                            <td>
                                                <c:choose>
                                                    <c:when test="${r.reviewType == 'PRODUCT'}">${r.productName}
                                                    </c:when>
                                                    <c:otherwise>${r.serviceType}</c:otherwise>
                                                </c:choose>
                                            </td>
                                            ...
                                            <td>
                                                <c:choose>
                                                    <c:when test="${r.verified}">
                                                        <span style="color:var(--rv-success-light);">✔</span>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <span style="color:var(--rv-text-secondary);">—</span>
                                                    </c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td>
